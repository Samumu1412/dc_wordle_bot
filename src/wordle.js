...
const { answers } = require('./constant')
...
function writeToCSVFile(newData) {
  ...
  let csvContent =
    'data:text/csv;charset=utf-8,' + newData.map((e) => e.join(',')).join('\n')
        ...
        if (data[i][0] == msg.author.id) {
          if (PlayedToday(data[i][3])) {
            if (!msg.content.includes('ISAIDSTARTANEWGAME')) {
              msg.reply(
                'You have already played a game, please come back tomorrow'
              )
              return
            }
          }
